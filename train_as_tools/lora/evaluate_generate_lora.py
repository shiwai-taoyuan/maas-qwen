...
def init_model_and_tokenizer(qwen2_7b_model_path, lora_model_dir=None):
    if lora_model_dir and not check_lora_model(lora_model_dir):
        print(f"{lora_model_dir}中的lora模型文件不完整！")
        raise 
...
if __name__ == '__main__':
    valid_data_path ='./data/tag_generate/naturalenvironment/valid/valid_data.jsonl'
    if not os.path.isfile(valid_data_path):
        print("验证集文件地址错误！")
        raise

    model_path = "/path/to/local/models/Qwen2-7B-Instruct"
    lora_model_path = "./outputs/tag_generate/sft-qwen-v2-naturalenvironment-100epoch/checkpoint-3500"
    ...
    with open(valid_data_path,'r') as f:
        for line in f.readlines():
            ...
            gt_tags = json.loads(gt)['tags']
            try:
                tags = json.loads(response)['tags']
            except:
                tags = []

            logger.info(f"真实值={gt},模型预测值={response},信息:{str(conversations)}")
            print('真实标签:',gt_tags,'  模型标注标签:',tags)

            if all(item in gt_tags for item in tags):
                correct_num +=1
    ...
    logger.info(f"{lora_model_path}验证结果：总数量={total_num},正确数量={correct_num},正确率={round(correct_num/total_num,2)}!")
