...
def chat(model, tokenizer, query: str, history: List[Dict] = None, **kwargs):
    ...
    history_.append({"role": "user", "content": query})
...
def init_model_and_tokenizer(qwen2_7b_model_path, lora_model_dir=None, quantization_config=''):
    if lora_model_dir and not check_lora_model(lora_model_dir):
        raise Exception(f"{lora_model_dir}中的lora模型文件不完整！")

    if not quantization_config:
        raise Exception(f"未设置quantization_config参数！")
...
if __name__ == '__main__':
    ...
    model, tokenizer, origin_model = init_model_and_tokenizer(model_path, lora_model_path, quantization_config)
